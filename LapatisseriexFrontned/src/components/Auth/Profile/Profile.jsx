...
const Profile = () => {
  ...
  // Handle editMode from navigation state
  useEffect(() => {
    if (routerLocation.state?.editMode) {
      setIsEditMode(true);
      // Clear the state to prevent re-triggering if user navigates back
  ...
  // Ensure we always return to view mode when leaving the Profile page
  // This clears any persisted edit state so navigating away and back resets to view mode
  useEffect(() => {
    return () => {
      try {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            {/* Email Verification Component - Below Email Input */}
            {formData.email && (
              <div className="mt-3">
                <EmailVerification 
