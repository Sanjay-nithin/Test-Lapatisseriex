...
export const AuthProvider = ({ children }) => {
  ...
  const verifyOTP = async (otp) => {
    ...
    try {
      ...
      // Only show first-time setup modal for new users after signup (not after login)
      if (response.data.isNewUser && (!response.data.user.name || !response.data.user.dob)) {
        setIsAuthPanelOpen(false); // Close auth panel
        setShowFirstTimeModal(true); // Show first-time setup modal
      } else {
  ...
  // Change auth type (login, signup, otp)
  const changeAuthType = (type) => {
    setAuthType(type);
    setAuthError(null);
  };

  // First-time setup modal controls
  const openFirstTimeModal = () => setShowFirstTimeModal(true);
  const closeFirstTimeModal = () => setShowFirstTimeModal(false);
